...
应用支持通过**真正的iCloud CloudKit**同步笔记到多台Mac设备：
...
- ✅ **真正的云同步** - 使用Apple CloudKit，数据存储在iCloud
- ✅ **增量同步** - 只同步修改过的笔记，节省流量
- ✅ **冲突解决** - 自动处理多设备编辑冲突（最后修改优先）
- ✅ **端到端加密** - 笔记内容加密后上传到云端
- ✅ **自动降级** - 如果CloudKit不可用，自动使用本地模拟实现
...
**在第二台设备上：**
1. 确保使用相同的Apple ID登录iCloud
2. 安装并打开encnotes
3. 启用iCloud同步
4. 点击"从iCloud拉取"下载笔记

**详细使用指南**：查看 [docs/CLOUDKIT_USAGE.md](docs/CLOUDKIT_USAGE.md)

**注意**：
- 如果未安装CloudKit依赖，应用会使用本地模拟实现（不会真正同步到云端）
- 安装依赖后需要重启应用才能生效
