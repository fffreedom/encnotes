...
#### 全面加密保护
- ✅ **启动时密码解锁**：首次启动时设置加密密码，之后每次启动需要输入密码解锁
- ✅ **自动解锁**：密码保存在系统钥匙串，支持自动解锁（可选）
- ✅ **修改密码**：支持修改加密密码，自动重新加密所有笔记
- ✅ **锁定功能**：可随时锁定笔记，需要重新输入密码才能访问
- ✅ **全部笔记加密**：所有笔记内容使用 AES-256 加密存储在本地数据库

#### 加密技术细节
- 🔒 **AES-256 加密**：使用业界标准的 AES-256-CBC 加密算法
- 🔑 **PBKDF2 密钥派生**：使用 PBKDF2-HMAC-SHA256（100,000次迭代）从密码派生加密密钥
- 🔐 **随机盐值和IV**：每个密码使用独立的随机盐值，每次加密使用随机IV
- 💾 **钥匙串存储**：加密密钥安全存储在 macOS 系统钥匙串中
- 🔄 **iCloud 同步加密**：同步到 iCloud 的笔记也是加密状态

#### 安全特性
- ⚠️ **密码不可恢复**：忘记密码将无法恢复笔记（类似 Mac 备忘录）
- 🛡️ **端到端加密**：只有您知道密码，Apple 和其他人无法访问您的笔记内容
- 🔒 **本地加密存储**：笔记在本地数据库中以加密形式存储
- 🔐 **密码强度要求**：密码至少8个字符

#### 用户界面
- 🎨 **精美的密码对话框**：设计美观的密码输入界面
+- 🚪 **解锁可退出**：解锁密码对话框新增"退出"按钮，用户可直接退出程序
- 📝 **首次设置向导**：首次使用时引导设置密码
- 🔄 **修改密码功能**：在"安全"菜单中提供修改密码选项
- 🔒 **锁定快捷键**：Ctrl+Shift+L 快速锁定笔记

### 📦 新增文件
- `encryption_manager.py`：加密管理器，处理所有加密相关操作
- `password_dialog.py`：密码对话框，包括解锁、设置和修改密码界面

### 🔧 技术改进
- 修改 `note_manager.py`：集成加密功能，自动加密/解密笔记内容
- 修改 `main_window.py`：添加密码管理和锁定功能
- 添加 `re_encrypt_all_notes()` 方法：支持修改密码后重新加密

### 📚 依赖更新
- 新增依赖：`cryptography`（加密库）
- 新增依赖：`keyring`（钥匙串访问）
...
#### 第一行自动大标题格式
- ✅ **自动格式化**：编辑笔记时，第一行内容自动以大标题格式显示（22号字体，粗体）
- ✅ **智能检测**：只在第一行不是标题格式时才应用，避免重复格式化
- ✅ **无感体验**：自动应用格式，无需手动操作
...
#### 修复切换笔记时标题被错误更新的问题
- ✅ **问题原因**：在 `save_current_note()` 中使用 `currentItem()` 获取列表项，但在 `on_note_selected()` 调用保存时，`currentItem()` 已经变成了新笔记，导致用旧笔记的标题更新了新笔记
- ✅ **解决方案**：根据 `self.current_note_id` 遍历查找对应的 item，而不是使用 `currentItem()`
- ✅ 现在切换笔记时，每个笔记的标题都会正确保存和显示

### 🔧 技术改进
- 改进了 `save_current_note()` 方法，使用 `item.data(Qt.ItemDataRole.UserRole)` 匹配 note_id
- 避免了因 `currentItem()` 状态变化导致的错误更新
...
#### 修复切换笔记时标题重叠问题
- ✅ **根本原因**：`save_current_note()` 中使用 `setText()` 更新标题，但由于使用了自定义 widget，`setText()` 会在 widget 上层再绘制文本，导致标题重叠
- ✅ **解决方案**：获取自定义 widget 中的 QLabel，直接更新 QLabel 的文本，而不是调用 `setText()`
- ✅ 现在切换笔记时，标题会正确更新，不会出现重叠现象

### 🔧 技术改进
- 改进了 `save_current_note()` 方法，正确处理自定义 widget 的更新
- 通过 `itemWidget()` 获取 widget，然后通过 `layout().itemAt(0)` 获取标题 QLabel
- 使用 `isinstance()` 检查确保获取的是 QLabel 对象
...
#### 去掉笔记列表选中时的边框
- ✅ 修复选中笔记时出现的边框问题
- ✅ 为 `QListWidget::item` 添加 `border: none` 和 `outline: none`
- ✅ 设置 `QListWidget` 的焦点策略为 `NoFocus`，避免焦点边框
- ✅ 明确设置所有状态（selected、hover、focus）的边框为 none

### 🎨 样式优化
- 优化了笔记列表的CSS样式，确保选中状态下没有任何边框
- 改进了视觉效果，选中时只显示黄色背景，没有边框干扰
...
#### 笔记列表显示优化
- ✅ 去掉标题的文本框边框，使标题和预览内容看起来是一体的
- ✅ 减小标题和预览文字之间的间距（从4px改为2px），显示更紧凑
- ✅ 为QLabel添加明确的样式：无边框、透明背景、无内边距
- ✅ 禁用文本交互（`NoTextInteraction`），避免意外选中文本

### 🔧 技术改进
- 优化了笔记列表item的CSS样式
- 改进了QLabel的显示效果，避免出现边框
...
#### 笔记列表标题重叠问题（最终修复）
- ✅ 优化 widget 删除顺序：先收集所有 widget，再统一删除
- ✅ 在 `clear()` 之前确保所有 widget 完全删除
- ✅ 使用 `processEvents()` 在 `clear()` 之前处理删除事件
- ✅ 避免 `clear()` 后 widget 引用失效的问题

### 🔍 问题分析
- 切换笔记时调用的是 `on_note_selected()`，不会调用 `load_notes()`
- 标题重叠发生在切换文件夹/标签时（调用 `load_notes()`）
- 之前的代码在 `clear()` 之后调用 `processEvents()`，导致 widget 引用失效
...
#### 笔记列表标题重叠问题（终极修复）
- ✅ 使用 `QApplication.processEvents()` 强制立即处理删除事件
- ✅ 调用 `widget.setParent(None)` 解除父子关系
- ✅ 确保旧 widget 在添加新 widget 前完全删除
- ✅ 彻底解决切换笔记时标题覆盖的问题

### 🔧 技术改进
- 改进了 Qt widget 生命周期管理
- 使用同步方式处理异步删除事件
...
#### 笔记列表标题重叠问题（彻底修复）
- ✅ 修复切换笔记时标题仍然重叠的问题
- ✅ 在删除widget前先调用 `setItemWidget(item, None)` 解除关联
- ✅ 避免 `deleteLater()` 异步删除导致的widget残留
- ✅ 确保切换文件夹/标签时UI完全刷新
...
#### 笔记列表显示问题
- ✅ 修复切换笔记时标题重叠的问题
- ✅ 在清空笔记列表前手动删除所有自定义widget
- ✅ 使用 `deleteLater()` 确保widget被正确释放
- ✅ 避免QListWidget的clear()方法遗留widget对象
...
#### 侧边栏布局调整
- ✅ 将"最近删除"移到iCloud分组下面，与文件夹并列显示
- ✅ 文件夹和标签点击时显示黄色高亮背景（#FFE066）
- ✅ 悬停时显示浅黄色背景（#FFF4CC）

#### 笔记列表增强
- ✅ 每个笔记条目显示两行信息
  - 第一行：笔记标题（15px，黑色）
  - 第二行：修改时间 + 内容预览（12px，灰色 #888888）
- ✅ 内容预览自动截取笔记正文第一行
- ✅ 超长内容使用省略号（...）
- ✅ 笔记列表项增加内边距，提升可读性
- ✅ 修复笔记列表标题和预览文字重叠问题（设置固定高度60px）
...
- 🎨 **全新的侧边栏布局**
  - 顶部显示 "☁️ iCloud" 分组标题
  - 所有文件夹缩进显示，表示属于iCloud
  - 移除"收藏"功能（简化界面）
  - "最近删除"与iCloud并列显示
  - 新增"🏷️ 标签"分组，与iCloud并列
...
- `get_note_tags()` - 获取笔记的所有标签
- `get_notes_by_tag()` - 获取带有指定标签的笔记
- `get_tag_count()` - 获取标签下的笔记数量

#### 界面优化
- 侧边栏添加样式，更接近Mac备忘录
- 不可选中的分组标题（iCloud、标签）
- 文件夹和标签缩进显示
- 工具栏新增"新建标签"按钮（Ctrl+T）

### 📝 使用说明

#### 标签使用
1. 点击工具栏"🏷️ 新建标签"或按 Ctrl+T 创建标签
2. 右键点击标签可以重命名或删除
3. 点击标签查看该标签下的所有笔记
4. 标签名称后显示笔记数量
